...
An MCP server wrapping this client code is available in `.mcp-server-openalex/`. When running as a Claude Code MCP server, the 7 OpenAlex tools (`openalex_search_works`, `openalex_author_works`, etc.) are callable directly — no Python boilerplate needed. See [`docs/mcp-server.md`](../../../docs/mcp-server.md) for details.
