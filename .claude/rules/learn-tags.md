...
If `MEMORY.md` does not exist, create it using the Knowledge Base template below. To choose the right variant: check the project's `CLAUDE.md` for keywords like "LSE", "course", "workshop", or "teaching" → use the **teaching** template. Otherwise → use the **research** template (default).
...
For LSE or workshop projects, use these sections instead:
