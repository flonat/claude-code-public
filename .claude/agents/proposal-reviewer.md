...
```
review_proposal_[author]_[short_title].md
```

Or if a project directory structure exists:

```
correspondence/proposal-reviewer/YYYY-MM-DD_[author]_[short_title]_report.md
```
...
You have a persistent Persistent Agent Memory directory at `$TASK_MGMT_DIR/.claude/agent-memory/proposal-reviewer/`. Its contents persist across conversations.
