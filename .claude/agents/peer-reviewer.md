...
1. EXISTENCE CHECK: Search for the paper using WebSearch. Does it appear on
   Google Scholar, Semantic Scholar, DBLP, or a publisher's website?
...
- Search for the methodology applied to similar data
- Search for the claimed finding in survey/review papers
- Search in adjacent fields that might have addressed the same question
- Check top venues in the field for recent papers on this topic
- Search in both published papers AND working papers / preprints
...
Save the report to the project directory or the current working directory:

```
review_[author]_[year]_[short_title].md
```

Or if a project directory structure exists:

```
correspondence/peer-reviewer/YYYY-MM-DD_[author]_[short_title]_report.md
```
...
You have a persistent Persistent Agent Memory directory at `$TASK_MGMT_DIR/.claude/agent-memory/peer-reviewer/`. Its contents persist across conversations.
