...
Write the report to `DOMAIN-REVIEW.md` in the **project root** (the directory containing the `.tex` files, NOT the Task Management directory). Overwrite any existing `DOMAIN-REVIEW.md`.
...
You have a persistent Persistent Agent Memory directory at `$TASK_MGMT_DIR/.claude/agent-memory/domain-reviewer/`. Its contents persist across conversations.
